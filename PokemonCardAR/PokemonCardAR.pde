...
void setup() {
  ...
  {
     if(!name.equals("camera_para.dat"))pokemon.add(new Pokemon(name, this)); 
  }
...
void draw() {
  ...
  if(!cameraSelected) 
  {
    ...
    if(pokemonLoad.size() <= 6 && testPok && pokemon.get(i).testAndDraw())pokemonLoad.add(i); 
